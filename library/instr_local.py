...
class kikusui(object):
    ...
    def __init__(self, resourcemanager, gpib, isUSB=True, isuse=False):
        ...
        if self._isUse == True:
            ...
            try:
                if isUSB:
                    # USB0::0x0957::0x1A07::SN????????::INSTR
                    self._PLZ = self._rm.get_instrument("USB0::0x0957::0x1A07::" + gpib + "::INSTR")
                else:
...
class sourcemeter(object):
    ...
    def ReadVoltage(self, samples=1, channel=1, avg_mode='MOV', filt='ON', nplc=1):
        if self._isUse == True:
            if (channel == 1):
                ...
                self._SMU.write(":FUNC 'VOLT:DC';:VOLT:NPLC %f;:AVER:TCON %s;COUN %d;STAT %s" % (
                    nplc, avg_mode, samples, flit))
    ...
    def ReadCurrent(self):  # , samples = 1, channel = 1, avg_mode = 'MOV', filt = 'ON', nplc = 1):
        if self._isUse == True:
            ...
            self._SMU.write(":ABOR")
            self._SMU.write(":ARM:COUN 1;SOUR TIM")
            self._SMU.write(":FUNC 'CURR:DC';:CURR:NPLC %f;:AVER:TCON %s;COUN %d;STAT %s" %
    ...
    def __init__(self, resourcemanager, gpib, isUSB=False):
        ...
        try:
            if isUSB == False:
                ...
                # USB0::0x0957::0x1A07::SN????????::INSTR
                self._VOLTMETER = self._rm.get_instrument(
                    "USB0::0x0957::0x1A07::" + gpib + "::INSTR")
