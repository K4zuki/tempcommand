...
#            value="W:\\Tokyo\\Data\\Design Center\\Nori2\\Evaluation\\OL1_02.txt")
        ...
        self.nextrely -= 1
        self._isUSB_dmm1= self.add(npyscreen.CheckBox, value = False, name="34461A", relx=40, width=35)

#        self._isuse_dmm2 = self.add(npyscreen.CheckBox, value = True, name="Use DMM2", width=35)
        ...
        self._isuse_kikusui = self.add(npyscreen.CheckBox, value = False, name="Use Kikusui", width=35)
...
#        self._isuse_serial = self.add(npyscreen.CheckBox, value = False, name="Use Ser-I2C", width=35)
        ...
        self._shell = self.add(npyscreen.MultiLineEdit, scroll_end=True, value = ">>>\n", name="log:",
...
#        self._outfile.write( os.environ['COMPUTERNAME']+"\n" )
        ...
        self._outfile.write( monthname.monthname()+"\\"+todaydetail.strftime("%H.%M.%S")+".chamber.csv"+"\n" )
...
#            npyscreen.notify_confirm(str(self._cypress),title="REPORT",editw=1)
            ...
            self._logfile.write("failed to load Cypress I2C\n")
        else:
#            self._i2c  = usbio.usbio.I2C(self._cypress, 0x90)#0x48/7bit=0x90/8bit
        ...
        try:
            ...
            self._logfile.write("no VISA connected\n")
            
        self.display()
        currentvolt=3.7

        self._logfile.write( str( self._i2c )+"\n" )
        self._logfile.write( str(self._cypress)+"\n" )
        self._logfile.write( str( self._uli2c )+"\n" )
        self._logfile.write(str(chan0)+'\n')
        self._logfile.write(str(chan1)+'\n')
        ...
        try:
            ...
            npyscreen.notify_confirm("".join(traceback.format_tb(sys.exc_info()[2]))+": "
                +str(sys.exc_info()[1]),title="ERROR REPORT",editw=1)
            self._logfile.write("=-=-=-=-= using dummy instruments =-=-=-=-=\n")
            self._logfile.write("E3640A: "+str(self.E3640A)+"\n")
            self._logfile.write("Chamber: "+str(self.Chamber)+"\n")
            self._logfile.write("A34401A: "+str(self.A34401A)+"\n")
        else:
        ...
        if self._isuse_dmm2.value:
            try:
                ...
                npyscreen.notify_confirm("".join(traceback.format_tb(sys.exc_info()[2]))+": "
                    +str(sys.exc_info()[1]),title="ERROR REPORT",editw=1)
                self._logfile.write("A34970A: "+str(self.A34970A)+"\n")

        self.K2400 = instr_local.dummy()
        if self._isuse_smu.value:
            try:
                self.K2400 = instr_local.sourcemeter(rm,self._smu.get_value(),self._isuse_smu.value)
            except:
                npyscreen.notify_confirm("".join(traceback.format_tb(sys.exc_info()[2])),
                    title="ERROR REPORT",editw=1)
                self._logfile.write("K2400: "+str(self.K2400)+"\n")
        ...
        if self._isuse_kikusui.value:
            try:
                ...
                npyscreen.notify_confirm("".join(traceback.format_tb(sys.exc_info()[2])),
                    title="ERROR REPORT",editw=1)
                self._logfile.write("PLZ164: "+str(self.PLZ164)+"\n")

        self.mbedI2C = instr_local.dummy()
        if self._isuse_serial.value:
            try:
                self.mbedI2C = instr_local.serial_i2c(self._serial.get_value(),isuse=self._isuse_serial.value)
            except:
                npyscreen.notify_confirm("".join(traceback.format_tb(sys.exc_info()[2]))+": "
                    +str(sys.exc_info()[1]),title="ERROR REPORT",editw=1)
                self._logfile.write("mbedI2C: "+str(self.mbedI2C)+"\n")
#        else:
#            self.mbedI2C = instr_local.dummy()
#        self._logfile.write("mbedI2C: "+str(self.mbedI2C)+"\n")

        self._script_fullname=self._scrfilename.get_value()
        self._script_basename="\\".join(self._script_fullname.split("\\")[:-1]) #path\to\scriptfile(windows only)
        self._logfile.write("script: "+self._script_fullname+"\n")
        self._logfile.write("base: "+self._script_basename+"\n")
        ...
        try:
            self.sendmail(self._sendto.get_value(),self._sendsrv.get_value(),info,self._isuse_email.value)
        except:
            npyscreen.notify_confirm("".join(traceback.format_tb(sys.exc_info()[2]))+": "
...
#        from email.mime.multipart import MIMEMultipart
    ...
    def _temp(self,temp='30'):
        ...
        self._logfile.write( 'reached target')
    ...
    def _sample(self,dummy='-1'):
        ...
        self._logfile.write( 'sample triggered')
    ...
    def _suspend(self,dummy='-1'): # SUSPEND
        self._logfile.write( 'suspend triggered->')
    ...
    def _volt(self,volt='3.0'): # VOLT x.x : sets supply voltage to be x.xV
        ...
        self._logfile.write( 'voltage set: '+volt+' V')
    ...
    def _kikusui(self,current='1e-3'):
        self._logfile.write( 'current set: '+current+' A')
    ...
    def _base(self,baseaddr='90'): # BASE xx : set 8bit base address to be xx (HEX)
        ...
        self._logfile.write( 'i2c slave address set: %02X'%(baseaddr))
...
#        self._i2c = usbio.usbio.I2C(self._cypress, baseaddr)
    ...
    def _register(self,argument):
        ...
        self._logfile.write( 'reg address = 0x%02X, data = 0x%02X' %( i2creg ,i2cdata ))
    ...
    def _ulibase(self,argument):#UBASEx[+y]=zz -> channel=x[and y], baseaddress=zz
        ...
        for f in ulichan:
            ...
            self._logfile.write( 'i2c slave address set: 0x%02X of channel u%d\n' %(ulibase, int(f)))
        return 0
    ...
    def _uliregister(self,argument):#UREGx[+y]:aa=bb -> channel=x[and y],reg=aa,data=bb
        ...
        channels=ulichan.split('+')
#        self._logfile.write( str(channels)+'\n')
        if len(channels)>1:
            ulichan=[channels[0],channels[1]]
        else:
            ulichan=[channels[0]]
        ...
        for f in ulichan:
            ...
            self.shellResponse( 'channel = u%d, reg address = 0x%02X, data = 0x%02X'
                %( int(f), ulireg, ulidata ))
            self._logfile.write( 'channel = u%d, reg address = 0x%02X, data = 0x%02X\n'
                %( int(f), ulireg, ulidata ))
        return 0
    ...
    def _ulicall(self,conf): #UCALL(x=filename) load i2c setting file "filename" and write them all into device under uli module, channel=x
        ...
        script = scr.read()
        scr.close()
        self._logfile.write('ch u'+channel+': reading i2c config file: '+conf+"\n")
...
#        self._logfile.write( 'i2c slave address set: 0x'+str(baseaddr))
        self.shellResponse( 'i2c slave address set: 0x'+str(baseaddr))
        self._logfile.write( 'i2c slave address set: 0x'+str(baseaddr)+' of channel s%d'
            %(ch))
        return 0
    ...
    def _serialregister(self,argument):
        ...
        self._logfile.write(str(packet))
        return 0
    ...
    def _serialchannel(self,channel):
        ...
        ch=self.mbedI2C.getChannel()
        self._logfile.write( 'set channel to s'+str(channel))
    ...
    def _chanset(self,channels):
        ...
        self._logfile.write( 'set channels(@'+channels.replace('+',',')+')')
...
#        self.isInLoop.append(True)
        self._logfile.write( str(self.isInLoop) )
        var=argument.split(";")
        self._logfile.write( str(var))
        lst=var[0].split("+")
        self._logfile.write( str(lst))
        for var[0] in lst:
            self._logfile.write( str(var[1])+str(var[0]))
    ...
    def _loop(self,dummy='-1'):
        if (self.isInLoop!=[]):#==True):
            ...
            self._logfile.write( 'end of loop')
        return 0
